                        ...
                        <div class="box box-primary">
                            ...
                            <div class="box-header with-border no-padding-h-b row">
                                ...
                                <div class="col-sm-3 col-lg-2 no-padding-h sidebar-fileMan-col">
                                    ...
                                    <div ng-if='scope=="anonymous"' class="sidebar-file-manager @{{scope=='anonymous'?'side-active':''}}"  ng-click="setRepo('anonymous')" >
                                        <i class="fa fa-file " aria-hidden="true"></i>
                                        <button type="button" class="btn btn-info"  data-title="anonymous"  >
                            ...
                            <div class="col-sm-9 col-lg-10 no-padding-h fmActions">
                                <div class="row">
                                    <div class=" @{{lengthObj(infowindow)>0?'col-md-8':'col-md-12'}}">
                                        ...
                                        <button data-title="Add New Folder" class="btn btn-primary"  ng-disabled="   scope=='trash' || scope=='anonymous'"  ng-click="addFolder()">
                                            <i class="fa fa-folder " title="Add Folder" aria-hidden="true"></i>
                                        </button>

                                        <button type="button" class="btn btn-primary" ng-disabled="  scope=='trash' || scope=='anonymous'" ng-click="addFilesPop()"   data-title="Add New File"  >
                                            <i class="fa fa-file " aria-hidden="true"></i>
                                        </button>
                                        <button type="button" ng-disabled="lengthObj(selected)<=0" ng-if="scope!='anonymous'" class="btn btn-danger" ng-click="deleteSelected()" 
                                         data-title="Deleted selected"  >
                                            <i class="fa fa-trash " aria-hidden="true"></i>
                                        </button>
                                        <button type="button" ng-if="scope!='anonymous'" ng-disabled="lengthObj(selected)<=0 || scope=='shared' || scope=='trash'" class="btn btn-danger"
                                         ng-click="copySelected()" data-title="Copy selected"  >
                                            <i class="fa fa-copy " aria-hidden="true"></i>
                                        </button>
                                        <button ng-if="scope!='anonymous'" type="button" ng-disabled="lengthObj(copied_items)<=0 || scope=='shared' || scope=='trash'" class="btn btn-danger" ng-click="pasteSelected()"  data-title="Paste selected"  >
                                        ...
                                        <button ng-if="scope!='anonymous'" type="button" class="btn btn-info"  data-toggle="modal" title="Share"  ng-click="sharePop()"    ng-disabled="lengthObj(selected)<=0   || scope=='trash'"    data-title="Share"  >
                                            <i class="fa fa-share " aria-hidden="true"></i>
                                        </button>
                                        <button ng-if="scope!='anonymous'" type="button" class="btn btn-info" ng-disabled="lengthObj(selected)<=0 || scope=='shared' || scope=='trash'"  ng-click="rename()"  data-title="Rename"  >
...
<div class="popoverfm" id="popoverfm" style="top:@{{popover.top}}px;left:@{{popover.left}}px;" ng-show="popover.showPopover">
    <div class="popoverinner" ng-click="popover.showPopover=false">
        ...
        <button ng-if="popover.TYPE==1" type="button" class="btn"  data-toggle="modal" title="Share"  ng-click="sharePop()"  ng-disabled="lengthObj(selected)<=0 || scope=='shared' || scope=='trash'"      >
...
<div class="toast " ng-repeat='ts in toastQueue' >
    @{{ts.msg}}
</div>
</section>
...
function swal(options,fn){
    var c=confirm(options.text);
    if(c){
        fn()
    }
}
...
console.log('Query variable %s not found %s', params, urlSearchParams.get(variable));
    ...
    $scope.toast=function(msg,time=2000){
          $scope.toastid= $scope.toastid+1;
           var tst={msg:msg,time:time,show:true,id: $scope.toastid};  
           $scope.toastQueue.push(tst);
          
           
        // $scope.showtoast=msg;
        $timeout(function(){
          for(let i in  $scope.toastQueue){
              console.log($scope.toastQueue[i],tst.id)
            if($scope.toastQueue[i].id==tst.id)
                {
                    $scope.toastQueue.splice(i, 1);
                }
        }
    ...
    $scope.init=function(){
        ...
        .then(function(response) {
            if(! response.data.status){
                $scope.toast(response.data.message,500);
                $scope.structure=[];
                return 1;
            }
    ...
    $scope.refresh =function(){
        ...
        $http({
            method: 'POST',
            url: base_url+"/init",
            data: Object.toparams({ 'parent_id' : $scope.parent_id,deep:$scope.deeplink.id,keyword:$scope.search_keyword,scope:$scope.scope,sort:$scope.sortobj.key,odr:$scope.sortobj.order}),
            headers: {'Content-Type': 'application/x-www-form-urlencoded'}
        })
        .then(function(response) {
            if( response.data.status){
                ...
                $scope.structure=[];
                $scope.toast(response.data.message,2000);
            }
...
$scope.checkDeepLink=function(){
    ...
    if(isShare && isShare=='anonymous'){
        var id=$scope.getQueryVariable("id");
        $scope.deeplink={id:id,repo:'anonymous'};
        $scope.setRepo("anonymous");
        return true;
    }
    return false;
}
